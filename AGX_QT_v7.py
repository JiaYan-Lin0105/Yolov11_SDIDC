...
def insert_coordinates_to_db(latitude, longitude, defect, time):
	...
	''')
    cursor.execute('INSERT INTO locations (latitude, longitude,defect,time) VALUES (?, ?, ?, ?)',
                   (latitude, longitude, defect, time))
    conn.commit()
    cursor.close()
    conn.close()
...
def visualize(img, bbox_array, tracker):
    ...
    if temp != []:
        ...
        mobilenet_predicted_class, mobilenet_confidence = predict_mobilenet(mobilenet_model, cropped_img, transform,
                                                                            device)
        ...
        img = cv2.putText(img, "id:" + str(track_id), (xmin - 50, int(ymin) - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                          (0, 255, 0), 2)
    # cv2.rectangle(img,(xmin,ymin),(xmax,ymax), box_color, 2)
    if clas in class_dict:
        label = f"{class_dict[clas]} ({mobilenet_class_name})"
    img = cv2.putText(img, " " + str(label) + " " + str(round(score, 2)), (xmin, int(ymin) - 5),
                      cv2.FONT_HERSHEY_SIMPLEX, 0.5, (105, 237, 249), 2)
    return img, track_id
